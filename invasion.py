...
class Invasion:
    ...
    def start_game(self):
        ...
        add_missile_event = pygame.USEREVENT + 1
        pygame.time.set_timer(add_missile_event, 250)
        add_cloud_event = pygame.USEREVENT + 2
        pygame.time.set_timer(add_cloud_event, 50)
        add_destroyer_event = pygame.USEREVENT + 3
        pygame.time.set_timer(add_destroyer_event, 500)
        add_ghost_event = pygame.USEREVENT + 4
        pygame.time.set_timer(add_ghost_event, 250)
        add_terminator_event = pygame.USEREVENT + 5
        pygame.time.set_timer(add_terminator_event, 250)
        add_halcon_event = pygame.USEREVENT + 6
        pygame.time.set_timer(add_halcon_event, 250)
        add_hunter_event = pygame.USEREVENT + 6
        pygame.time.set_timer(add_hunter_event, 1250)
        add_meteor_event = pygame.USEREVENT + 7
        pygame.time.set_timer(add_meteor_event, 1000)
        add_asteroid_event = pygame.USEREVENT + 8
        pygame.time.set_timer(add_asteroid_event, 2000)
        add_comet_event = pygame.USEREVENT + 9
        pygame.time.set_timer(add_comet_event, 10000)
        invoke_transition_event = pygame.USEREVENT + 10
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                elif event.type == add_missile_event:
                ...
                elif event.type == add_destroyer_event:
                ...
                elif event.type == add_comet_event:
                    new_spatial_rock = Spatial_Rock(COMET)
                    self.spatial_rocks.add(new_spatial_rock)
                    self.all_sprites.add(new_spatial_rock)

                elif event.type == add_meteor_event:
                    new_spatial_rock = Spatial_Rock(METEOR)
                    self.spatial_rocks.add(new_spatial_rock)
                    self.all_sprites.add(new_spatial_rock)

                elif event.type == invoke_transition_event:
                    self.stats.ships_left = self.stats.ships_left - 1
                    self.scoreboard.prep_ships()
                    # draw the transition
                    print("Creada la transition @")
            ...
            if (pygame.sprite.spritecollideany(self.player, self.missiles) \
                or pygame.sprite.spritecollideany(self.player, self.destroyers) \
                    or pygame.sprite.spritecollideany(self.player, self.spatial_rocks) \
                    )and isActive:
                print("invocadala condicion")
                ...
                # Stop the loop
                pygame.time.set_timer(invoke_transition_event, 500, True)
                isActive = False
